...
main() {
  print(
      "There will be generated 5 candidates to be the anagram of the given word.");
  ...
  for (var i = 0; i < candidates.length; i++) {
    if (isAnagram(givenWord, candidates[i])) {
      if (givenWord.contains(candidates[i])) {
        print(
            "Oh! Lucky you! The random word '${candidates[i]}' is exactly the same as the given word");
      } else {
        print(
            "The random word '${candidates[i]}' is Anagram of '${givenWord}'");
      }
    } else {
      print(
          "The random word '${candidates[i]}' is not Anagram of '${givenWord}'");
    }
  }
}
